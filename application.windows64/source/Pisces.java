...
public class Pisces extends PApplet {
...
public void setup() 
{
  ...
  bruce = new Shark(0, PApplet.parseInt(random(height)));
  ...
  {
    fishList[i] = new Fish(PApplet.parseInt(random(width)), PApplet.parseInt(random(height)));
    fishList[i].targets = fishList;
    fishList[i].maxSpeed = random(1.5f) + 2.5f;
    fishList[i].c = color(0, 0, PApplet.parseInt(pow(fishList[i].maxSpeed, 4)));
  ...
  {
    dolphins[i] = new Dolphin(0, PApplet.parseInt(random(height)));
    dolphins[i].maxSpeed = random(1.5f) + 3.5f;
    dolphins[i].c = color(PApplet.parseInt(random(50,100)),0,155);
...
public void draw() 
{
  ...
  if(bruce.lives() && !dolphins[0].lives())
  {
    if(bruce.longLife())
    {
      PVector goal = new PVector(width, PApplet.parseInt(random(height)));
      for(int i = 0; i < dolphins.length; i++)
      {
  ...
  if(m%10000 == 0)
  {
    for(int i = 0; i < leaders.length; i++)
    {
      ...
      fishList[index].c = color(0, 0, PApplet.parseInt(pow(fishList[i].maxSpeed, 4)));
...
class Bubble extends Kinematics 
{
    ...
    Bubble(int c) 
    {
        ...
        pos = new PVector(PApplet.parseInt(random(width)), height);
...
class Shark extends Kinematics
{
  ...
  Shark(int x, int y) 
  {
    ...
    goal = new Kinematics(width, PApplet.parseInt(random(height)));
    last_update = millis();
    life_time = millis();
    exists = true;
  }
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Pisces" };
    if (passedArgs != null) {
